...
export default function ShoppingCart() {
  const { cart, isLoading, updateCartItem, deleteCart } = useCart();
  ...
  const { subtotal, delivery, discount, total } = useMemo(() => {
  ...
  }, [cart?.total_price]); // ← sirf total_price change hone pe recompute
  ...
  return (
    <div className="w-full px-4 py-6">
      <div className="max-w-7xl mx-auto mb-6">
        ...
        <Heading level={3} data-testid="cart-page-title">
          Shopping Cart
        </Heading>
      </div>
      ...
      <Section containerSize="full" className="relative w-full flex py-4">
        <div className="w-full grid grid-cols-1 md:grid-cols-[60%_40%] gap-6 items-start">
          ...
          <Card
            variant="glass"
            hover={false}
            className="w-full p-6 md:p-8 md:sticky md:top-32"
              ...
              {discount > 0 && (
                <div className="flex justify-between items-center">
                  <Text className="flex items-center gap-1.5">
                    <Tag size={14} /> Discount (10%)
                  </Text>
                  <Text className="font-semibold text-emerald-600 dark:text-green-400">
                    −₹{discount}
                  </Text>
                </div>
              )}
